...
- **Gradle wrapper** — check `git ls-files gradle/wrapper/gradle-wrapper.jar` actually returns the jar. Some repos gitignore it, and then `./gradlew` does not exist after a CI checkout and *every* workflow fails on its first step. If it is untracked, either commit it (Gradle's own recommendation — pair it with `gradle/actions/wrapper-validation` to address the supply-chain concern that motivates ignoring it) or pin `gradle-version` on `gradle/actions/setup-gradle` and invoke `gradle` instead of `./gradlew`. Ask which; do not silently un-ignore a file the project deliberately excluded.
